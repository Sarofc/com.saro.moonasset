...
namespace Saro.XAsset.Build
{
    ...
    public class CustomGroup
    {
        ...
        //[Tooltip("TODO 测试 远端资源 预加载")]
        //public bool preloadRemote;
        ...
        [HideInInspector]
        public List<RuleCustomAsset> assets = new List<RuleCustomAsset>();
        ...
        public string GetCustomBundleName(Manifest manifest)
        {
            foreach (var asset in assets)
            {
                ...
                if (manifest.CustomAssetMap.TryGetValue(name, out var bundle))
                {
                    return bundle.name;
                }
            }
        ...
        public List<RuleCustomAsset> GetAssets()
        {
            assets.Clear();
            for (int i = 0; i < searchPaths.Length; i++)
            {
                ...
                else // searchPath 是文件夹的情况
                {
                    ...
                    for (int j = 0; j < originAssets.Length; j++)
                    {
                        ...
                        assets.Add(new RuleCustomAsset
                        {
                            name = originAsset,
                            dir = i,
                        });
